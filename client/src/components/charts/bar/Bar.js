...
export default function Bar({ barData }) {
  ...
  useEffect(() => {
    ...
    const data = barData.reduce((acc, cur) => {
      if (!acc[cur.Year])
        acc[cur.Year] = [];
    ...
    const drawChart = () => {
      ...
      function update(data) {
        const clear = [
          ...new Map(data.map(obj => [`${obj.Code}:${obj.Area}`, obj]))
            .values()
        ];
        const topResults = clear.sort((a, b) => { return parseInt(b.Value) - parseInt(a.Value) }).slice(0, 15);
